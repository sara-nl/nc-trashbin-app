...
class TrashbinMapper extends QBMapper
{
    ...
    {
        $this->logger->debug(" - TrashbinMapper::getItem($id, $timestamp, $user, $deletedBy) ");
    ...
    {
        $this->logger->debug(" - TrashbinMapper::getItems($id, $timestamp) ");
        $query = $this->db->getQueryBuilder();
        $query->select('*')
            ->from(self::TABLE_NAME)
            ->where($query->expr()->eq(self::TABLE_COLUMN_ID, $query->createNamedParameter($id)))
        ...
        $this->logger->debug(" - found " . count($items) . " trashbin items.");
        return $items;
    }
    ...
    {
        $this->logger->debug(' - TrashbinMapper::insertItem() ');
        $query = $this->db->getQueryBuilder();
    ...
    {
        $this->logger->debug(" - TrashbinMapper::deleteItems($id, $timestamp)");
        $query = $this->db->getQueryBuilder();
