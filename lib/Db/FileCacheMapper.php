...
class FileCacheMapper extends QBMapper
{
    ...
    {
        $this->logger->debug(" - FileCacheMapper::getItem($fileid) ");
        try {
    ...
    {
        $this->logger->debug(" - FileCacheMapper::getItems(path: $path, name: $name) ");
        try {
            $statement = $this->db->executeQuery(
                <<<'SQL'
                select distinct fc1.*, s.id as storage_id
    ...
    {
        $this->logger->debug(" - FileCacheMapper::getUserAndFAcountItemsForDeletedItem($fAccountFileid) ");
        try {
            $statement = $this->db->executeQuery(
                <<<'SQL'
                select distinct fc1.*,s.id as storage_id
    ...
    {
        $this->logger->debug(' - TrashbinMapper::insertItem() ' . print_r($filecacheItem, true));
        try {
    ...
    /**
     * Delete the filecache item with the specified fileid.
     * 
     * @var int fileid
     * @return int the number of items deleted
     */
    public function deleteItem(int $fileid): int
    {
        $this->logger->debug(" - FileCacheMapper::deleteItem($fileid)");
        $query = $this->db->getQueryBuilder();
        $query->delete(self::TABLE_NAME)
            ->where($query->expr()->eq(self::TABLE_COLUMN_FILEID, $query->createNamedParameter($fileid)));
        $result = $query->executeStatement();
        $this->logger->debug(" SQL: " . print_r($query->getSQL(), true));

        if (!$result) {
            $this->logger->error("Unable to delete filecache item: $fileid", ['app' => Application::APP_ID]);
        }
        return $result;
    }

    /**
     * Delete the filecache items for the specified name, path and storage.
     * 
     * @var string $name
     * @var string path
     * @var int storage
     * @return int the number of items deleted
     */
    public function deleteItems(string $name, string $path, int $storage): int
    {
        $this->logger->debug(" - FileCacheMapper::deleteItems($name)");
        $query = $this->db->getQueryBuilder();
        $query->delete(self::TABLE_NAME)
            ->where($query->expr()->eq(self::TABLE_COLUMN_NAME, $query->createNamedParameter($name)));
        $result = $query->executeStatement();
        $this->logger->debug(" SQL: " . print_r($query->getSQL(), true));

        if (!$result) {
            $this->logger->error('Unable to delete filecache items.', ['app' => Application::APP_ID]);
        }
        return $result;
    }

    /**
     * Returns the children of the specified filecache item.
     * 
     * @param array $item
     * @return array the children
     */
    public function getChildren(array $item): array
    {
        $fileid = (is_array($item) && isset($item[self::TABLE_COLUMN_FILEID])) ? $item[self::TABLE_COLUMN_FILEID] : 'not set';
        $this->logger->debug(" - FileCacheMapper::getChildren(fileid: $fileid)");
        try {
            $query = $this->db->getQueryBuilder();
            $query->select('*')
                ->from(self::TABLE_NAME)
                ->where($query->expr()->eq(self::TABLE_COLUMN_PARENT, $query->createNamedParameter($item[self::TABLE_COLUMN_FILEID])));
    ...
    {
        $this->logger->debug(" - FileCacheMapper::getTrashbinRootItem($storage) ");
        try {
            $query = $this->db->getQueryBuilder();
            $query->select('*')
                ->from(self::TABLE_NAME)
